...
Edits on February 11, 2017:
1) Enclosed the jQuery functionality into an IIFE for improved security.
2) Added API key to comply with Google Maps JavaScript API's requirement for an API key.
3) The original app used the HTML5 Geolocation API to detect the user's current location.  As of Google Chrome version 50 and up,
HTML5 Geolocation API has been deprecated for apps running on non-HTTPS hosts.  In light of this change, the app has been
changed to detect a user's location--and the 10 nearest LCBO stores--based on the postal code that the user has entered.
*/
...
(function() {
				  	...
				  	function(wine, index, array) {
				  		...
				  		// Wine description (includes wine name and price)
				  		 ...
				  		 var $details = $("<div>")
				  			...
				  			// Build button to get LCBO locations nearest to a specific postal code that store the wine.
				  		 ...
				  		 var $button = $("<div>")
				  		 	...
				  		 	.append(
				  		 		$("<button>")
				  		 			...
				  		 			.text("Find in stores nearest location")
	...
	/* Get a list of the 10 nearest LCBO locations to a specified postal code 
	that stock a specific product (by LCBO Product ID) */
	app.getLocationsWithProduct = function(productName, productId, postalCode) {
	  ...
	  var $h1 = $("<h1>").text("Closest stores to postal area " + postalCode);
	...
	$(function() {
		...
		/* Find the nearest 10 LCBO stores for the specified postal code
		where where a specific wine product can be purchased  */
		$("#winePairings").on("click", ".winePairingInfo button.findLocation", function() {
			...
			// Get the name of the wine product and the product ID, and get the entered postal code
			var productName = $(this).data("lcbo-product-name");
			var productId = $(this).data("lcbo-product-id");
			var postalCode = $("#postalCode").val();

			// Get the nearest 10 LCBO stores for the specified postal code 
			// where a specific wine product can be purchased
			app.getLocationsWithProduct(productName, productId, postalCode);
		});

		/* Display more information for the nearest 10 LCBO stores for a specified postal code
		with a specific wine product. */
